/**
 * Import missing Sherwin-Williams colors from the official SW Prism API.
 *
 * The API at api.sherwin-williams.com/prism/v1/colors/sherwin returns all SW
 * colors with name, color number, hex, RGB, LRV, Lab values, and color family.
 *
 * Usage: npx tsx scripts/import-sw-api-colors.ts [--dry-run]
 */
...
async function main() {
  ...
  const res = await fetch(
    "https://api.sherwin-williams.com/prism/v1/colors/sherwin?lng=en-US&_corev=7.10.0"
  );
  if (!res.ok) {
    console.error("Failed to fetch SW API:", res.status, res.statusText);
    process.exit(1);
  }
  ...
  const { data: brand } = await supabase
    .from("brands")
    .select("id")
    .eq("slug", "sherwin-williams")
    .single();

  if (!brand) {
    console.error("Sherwin-Williams brand not found in DB");
    process.exit(1);
  }
  ...
  const missing = activeColors.filter((c) => {
    return (
      ...
      !dbNumbers.has("SW " + c.colorNumber) &&
  ...
  console.log(`\nDone! Inserted ${inserted} new Sherwin-Williams colors.`);
  console.log(
    "\nNext steps:\n" +
