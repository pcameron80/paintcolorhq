...
/** Pull the whole feed, dedupe to one entry per handle. */
async function fetchFeed(): Promise<FeedEntry[]> {
  const byHandle = new Map<string, FeedEntry>();
  let offset = 1;
  let total = Infinity;
  while (offset <= total) {
    // offset-pagination is bounded to the first 10,000 records; size the last page
    // so offset+limit never exceeds that window.
    const limit = total === Infinity ? PAGE : Math.min(PAGE, total - offset + 1);
    if (total > 10000) { console.warn(`⚠️ feed has ${total} > 10,000 — offset paging can't reach the tail; switch to page-based query.`); }
    ...
    for (const p of page.resultList) {
      ...
      byHandle.set(handle, { handle, brandSlug, code: normCode(codes[codes.length - 1] ?? "") });
    }
    console.log(`  feed: ${byHandle.size} unique handles (offset ${offset}/${total})`);
...
async function fetchColors() {
  ...
  const rows: Array<{ id: string; slug: string; code: string; brandSlug: string; crawlAvailable: boolean | null }> = [];
  for (let from = 0; ; from += 1000) {
    const { data, error } = await supabase
      .from("colors").select("id, slug, color_number, brand_id, samplize_available")
      .in("brand_id", brandIds).order("id", { ascending: true }).range(from, from + 999);
    if (error) throw error;
    if (!data?.length) break;
    for (const c of data) rows.push({ id: c.id as string, slug: c.slug as string, code: normCode(c.color_number as string), brandSlug: slugById.get(c.brand_id as string)!, crawlAvailable: (c.samplize_available as boolean | null) });
...
async function main() {
  ...
  const pchqBrandCode = new Set(colors.filter((c) => c.code).map((c) => `${c.brandSlug}|${c.code}`));
  ...
  for (const c of colors) {
    const handle = bySlug.get(c.slug) ?? (c.code ? byBrandCode.get(`${c.brandSlug}|${c.code}`) : undefined);
