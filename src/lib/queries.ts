...
export async function getCrossBrandMatches(
  ...
  // Pre-computed match data lives at public/match-data/{colorId}.json,
  // generated by scripts/build-match-data.ts at build time. Reading the
  // file is ~free (deployment-bundled static asset on Vercel) compared
  // to a Supabase round-trip on every cold-cache page render.
  //
  // Falls back to the live Supabase query if the file is missing — this
  // keeps dev environments and any color added since last build working
  // gracefully.
  try {
    ...
    return JSON.parse(raw) as CrossBrandMatchWithColor[];
  } catch {
    // File not found or parse failed — fall back to live query
  }
